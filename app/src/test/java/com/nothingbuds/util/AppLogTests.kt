...
/**
 * End-to-end logging-pipeline tests: entries logged from every layer must reach
 * the same exportable sink (AppLog.snapshot), in order, under rotation and
 * concurrent writes — the exact input the log exporter reads.
 */
class AppLogTests {
    ...
    fun `entries from all layers reach the export snapshot`() {
        ...
        AppLog.d("BudsService", "[DIRAC][A1] TX DIRAC cmd=0xF01D payload=0300")
        AppLog.d("BudsService", "[DIRAC][A1] RX ACK cmd=0x4027 payload=00 ackStatus=0")
    ...
    @Test
    fun `buffer rotates at cap keeping newest`() {
        repeat(AppLog.MAX_ENTRIES + 100) { AppLog.d("Test", "line $it") }
        assertEquals(AppLog.MAX_ENTRIES, AppLog.size())
        val export = AppLog.snapshot().joinToString("\n")
        assertTrue(export.contains("line ${AppLog.MAX_ENTRIES + 99}"))
        assertTrue(!export.contains("line 0\n") && !export.startsWith("1 "))
    }
    ...
    fun `concurrent writes are thread-safe`() {
        ...
        assertTrue(snapshot.size <= AppLog.MAX_ENTRIES)
