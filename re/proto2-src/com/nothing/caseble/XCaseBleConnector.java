...
public final class XCaseBleConnector extends XBleConnector {
    ...
    public /* synthetic */ XCaseBleConnector(XCaseBleParser xCaseBleParser, String str, int i, DefaultConstructorMarker defaultConstructorMarker) {
        this((i & 1) != 0 ? new XCaseBleParser() : xCaseBleParser, (i & 2) != 0 ? "CaseBleWriter" : str);
    }
    ...
    public void initParams(XBluetoothDevice device) {
        ...
        if (getWriteUUID().length() == 0) {
            setWriteUUID("66555943-1810-45e6-8326-fc8ca3bc45ce");
        }
        if (getServiceUUID().length() == 0) {
            setServiceUUID(DEFAULT_SERVICE_UUID);
        }
        if (getNotifyUUID().length() == 0) {
            setNotifyUUID("66555943-1810-45e6-8326-fc8ca3bc45ce");
        }
    }
    ...
    public void printWriterLog(XCommand command, byte[] byteArray) {
        ...
        if (logger.isCanLogger(true)) {
            String str = "[CaseBle] -> Write " + getTag() + " [" + BleUtil.bytesToHex$default(BleUtil.INSTANCE, byteArray, false, 2, null) + "](" + byteArray.length + ")";
            String str2 = str;
    ...
    public void printReceiverLog(String command, byte[] byteArray) {
        ...
        int depth = logger.getDepth();
        if (logger.isCanLogger(true)) {
            String str = "[CaseBle] <- Receive " + getTag() + " [" + BleUtil.bytesToHex$default(BleUtil.INSTANCE, byteArray, false, 2, null) + "](" + byteArray.length + ")";
            String str2 = str;
            if (str2 == null || str2.length() == 0) {
                return;
            }
